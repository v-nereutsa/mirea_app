...
//  Easy Fitness
//
//  Created by Sam Mejlumyan on 27.06.17.
//  Copyright © 2017 @smejl. All rights reserved.
//
...
extension UIView {
  ...
  @discardableResult
    ...
    @discardableResult
    func pinEdgesToSuperviewEdges(top: CGFloat = 0, left: CGFloat = 0, right: CGFloat = 0, bottom: CGFloat = 0) -> [NSLayoutConstraint] {
        guard let superview = superview else {
            fatalError("There is no superview")
        }

        var constraints = [NSLayoutConstraint]()
        constraints.append(leftAnchor ~= superview.leftAnchor + left)
        constraints.append(rightAnchor ~= superview.rightAnchor - right)
        constraints.append(topAnchor ~= superview.topAnchor + top)
        constraints.append(bottomAnchor ~= superview.bottomAnchor - bottom)
        return constraints
    }
